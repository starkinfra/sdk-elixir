defmodule StarkInfra.IssuingHolder.Log do
  ...
  user, but it can be retrieved to check additional information on the IssuingHolder.
  ...
  ## Attributes (return-only):
    ...
    - `:type` [binary]: type of the IssuingHolder event which triggered the log creation. ex: "blocked", "canceled", "created", "unblocked", "updated"
  ...
  @doc """
  Receive a stream of IssuingHolder.Log objects previously created in the Stark Infra API
  ...
  ## Parameters (optional):
    ...
    - `:types` [list of binaries, default nil]: filter for log event types. ex: ["created", "blocked"]
    - `:holder_ids` [list of binaries, default nil]: list of IssuingHolder ids to filter logs. ex: ["5656565656565656", "4545454545454545"]
